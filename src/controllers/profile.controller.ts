...
export const getStudentProfile = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (cached) {
        return res.json({ success: true, student: JSON.parse(cached), source: 'cache' });
    }
    ...
    if (token && req.params.username) {
        ...
        try {
            const [gradesRes, attendanceRes] = await Promise.all([
                axios.get(`${GATEWAY_URL}/academics/grades?studentId=${targetUsername}`, { headers: { Authorization: token }, timeout: 2000 }).catch(() => ({ data: null })),
                axios.get(`${GATEWAY_URL}/academics/attendance?studentId=${targetUsername}`, { headers: { Authorization: token }, timeout: 2000 }).catch(() => ({ data: null }))
            ]);
